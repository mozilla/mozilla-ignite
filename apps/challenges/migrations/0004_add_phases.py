...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Adding model 'Phase'
        db.create_table('challenges_phase', (
        ...
        db.send_create_signal('challenges', ['Phase'])

        # Adding unique constraint on 'Phase', fields ['challenge', 'name']
        db.create_unique('challenges_phase', ['challenge_id', 'name'])

        # Adding field 'Submission.phase'
        db.add_column('challenges_submission', 'phase', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['challenges.Phase'], null=True, blank=True), keep_default=False)

        # Changing field 'Submission.challenge'
        db.alter_column('challenges_submission', 'challenge_id', self.gf('django.db.models.fields.related.ForeignKey')(to=orm['challenges.Challenge'], null=True))


    def backwards(self, orm):
        
        if not db.dry_run:
            if orm.Submission.objects.filter(challenge=None).exists():
                raise RuntimeError("Cannot reverse this migration. Submissions "
                                   "exist with no associated challenge.")
        
        # Changing field 'Submission.challenge'
        # Specifying a default here because there's no option not to, but
        # the check beforehand should ensure it isn't needed
        db.alter_column('challenges_submission', 'challenge_id', self.gf('django.db.models.fields.related.ForeignKey')(default=1, to=orm['challenges.Challenge']))
        
        # Removing unique constraint on 'Phase', fields ['challenge', 'name']
        db.delete_unique('challenges_phase', ['challenge_id', 'name'])

        # Deleting model 'Phase'
        db.delete_table('challenges_phase')

        # Deleting field 'Submission.phase'
        db.delete_column('challenges_submission', 'phase_id')
    ...
    models = {
        ...
        'challenges.submission': {
            ...
            'challenge': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['challenges.Challenge']", 'null': 'True', 'blank': 'True'}),
            ...
            'phase': ('django.db.models.fields.related.ForeignKey', [], {'to': "orm['challenges.Phase']", 'null': 'True', 'blank': 'True'}),
            'title': ('django.db.models.fields.CharField', [], {'unique': 'True', 'max_length': '60'})
        },
