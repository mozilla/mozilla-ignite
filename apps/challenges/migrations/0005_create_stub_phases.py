...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Make sure all submissions have an assigned phase.
        
        Assume that all submissions have an associated challenge.
        
        """
        for submission in orm.Submission.objects.filter(phase=None):
            phases = submission.challenge.phase_set.all()
            
            if phases:
                submission.phase = phases[0]
            else:
                p = orm.Phase.objects.create(name='(unnamed phase)',
    ...
    def backwards(self, orm):
        """Make sure all submissions have an assigned challenge.
        
        Assume all submissions have an assigned phase.
        
        """
        submissions = orm.Submission.objects.filter(challenge=None)
        for submission in submissions:
            submission.challenge = submission.phase.challenge
            submission.save()
