...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        try:
            project = orm['projects.Project'].objects.get(slug=IGNITE_PROJECT_SLUG)
        except ObjectDoesNotExist:
            project_data = {
                'name': 'Ignite Challenge',
                'slug': IGNITE_PROJECT_SLUG,
                'description': '',
                'long_description': '',
                'allow_participation': True,
                }
            project = orm['projects.Project'].objects.create(**project_data)

        end_date = datetime.datetime.now() + relativedelta(months=3)
        try:
            challege = (orm['challenges.Challenge'].objects
                        .get(slug=IGNITE_CHALLENGE_SLUG))
        except ObjectDoesNotExist:
            print ('WARNING: Challenge end date setup to %s. Amend this in '
                   'the admin' % end_date)
            challenge_data = {
                'title': 'Ignite Challenge',
                'slug': IGNITE_CHALLENGE_SLUG,
                'summary': '',
                'description': '',
                'end_date': end_date,
                'project': project,
                }
            challenge = orm['challenges.Challenge'].objects.create(**challenge_data)
        try:
            phase = orm['challenges.Phase'].objects.get(name=IDEATION_SLUG)
        except ObjectDoesNotExist:
            print ('WARNING: Phase end date setup to %s. Amend this in '
                   'the admin' % end_date)
            phase_data = {
                'name': IDEATION_SLUG,
                'end_date': end_date,
            ...
            phase = orm['challenges.Phase'].objects.create(**phase_data)
    ...
    models = {
        ...
        'challenges.phase': {
            ...
            'end_date': ('django.db.models.fields.DateTimeField', [], {'default': 'datetime.datetime(2012, 11, 10, 14, 14, 38, 746513)'}),
            'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
